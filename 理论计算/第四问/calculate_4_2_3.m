...
p1 = [0.10462, 0.0570443, 0.104619];
p2 = [0.10462, 0.0570443, 0.104619];
p3 = [0, 0, 0];
m = [6,5,6];
for i = 1:length(rate)
    ...
    for j = 1:length(r)
        ...
        Ps1(j) = (tau1(j)*(1 - tau2(j)) + tau2(j)*(1 - tau1(j))) / Ptr1(j);
        Ps2(j) = (tau2(j)*(1 - tau1(j)) + tau1(j)*(1 - tau2(j)) + tau3(j) - tau1(j)*tau3(j)*(1 - tau2(j)) - tau2(j)*tau3(j)*(1 - tau1(j))) / Ptr2(j);
        Ps3(j) = (tau3(j) + tau2(j)*(1 - tau1(j))-(1 - tau1(j))*tau2(j)*tau3(j)) / Ptr3(j);
