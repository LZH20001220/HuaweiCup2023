...
for i = 1:length(rate)
    ...
    for j = 1:length(r)
        ...
        Ps1(j) = (tau1(j) + tau2(j) - tau1(j)*tau2(j)) / Ptr1(j);
        Ps2(j) = (tau2(j) + tau1(j) + tau3(j) - tau1(j)*tau2(j) - tau1(j)*tau3(j) - tau2(j)*tau3(j)+tau1(j)*tau2(j)*tau3(j)) / Ptr2(j);
        Ps3(j) = (tau3(j) + tau2(j) - tau3(j)*tau2(j)) / Ptr3(j);
        
        S1(i, j) = (tau1(j)*(1 - tau2(j))* L_PLD * 8)/(T_slot *(1 - Ptr1(j)) + Ptr1(j)*Ps1(j)*Ts(i) + Ptr1(j)*(1 - Ps1(j))*Tc(i));
        S2(i, j) = (tau2(j)*(1 - tau1(j))* L_PLD * 8)/(T_slot *(1 - Ptr2(j)) + Ptr2(j)*Ps2(j)*Ts(i) + Ptr2(j)*(1 - Ps2(j))*Tc(i));
        S3(i, j) = (tau3(j)* L_PLD * 8)/(T_slot *(1 - Ptr3(j)) + Ptr3(j)*Ps3(j)*Ts(i) + Ptr3(j)*(1 - Ps3(j))*Tc(i));
        S(i, j) = S1(i, j) + S1(i, j) + S3(i, j);
        
%         S1rts(i, j) = (tau1(j)* L_PLD * 8)/(T_slot *(1 - Ptr1(j)) + Ptr1(j)*Ps1(j)*Tsrts(i) + Ptr1(j)*(1 - Ps1(j))*Tcrts(i));
%         S2rts(i, j) = (tau2(j)* L_PLD * 8)/(T_slot *(1 - Ptr2(j)) + Ptr2(j)*Ps2(j)*Tsrts(i) + Ptr2(j)*(1 - Ps2(j))*Tcrts(i));
%         S3rts(i, j) = (tau3(j)* L_PLD * 8)/(T_slot *(1 - Ptr3(j)) + Ptr3(j)*Ps3(j)*Tsrts(i) + Ptr3(j)*(1 - Ps3(j))*Tcrts(i));
